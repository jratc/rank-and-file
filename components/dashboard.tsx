    ...
    const handleUpdateTitle = async (targetListId: string) => {
        ...
        if (targetListId === 'temp-pending') {
            ...
            try {
                ...
                try {
                    const popResult = await detectAndPopulateList(newList.id, newList.title, newList.category);
                    if (popResult.populated) {
                        populatedCount = popResult.count;
                        // Use actual items which now come with real DB IDs
                        if (popResult.items && popResult.items.length > 0) {
                            newList.list_items = popResult.items;
                        } else {
                            newList.list_items = Array(popResult.count).fill({});
                        }
                    }
                } catch (popErr) {
                    console.error("Population failed", popErr);
                }

                // Replace temp list with real list
                setLists(prev => prev.map(l => l.id === 'temp-pending' ? newList : l));
                setExpandedListId(newList.id);

                if (populatedCount > 0) {
                    toast.success(`List created & populated with ${populatedCount} items!`);
                    // Trigger a reload of the component data to get the full items if needed, 
                    // or just let the user open it (which triggers fetch if we implemented it that way, 
                    // but we currently assume lists has all data? No, activeLists has data).
                    // To be safe, let's refresh.
                    router.refresh();
                } else {
                ...
                // Reset to temp state if failed? Or remove? Let's remove to avoid stuck state.
                setLists(prev => prev.filter(l => l.id !== 'temp-pending'));
                setExpandedListId(null);
            } finally {
                cleanup();
                // Focus search if NOT populated
                // If populated, the user probably wants to see the items, so maybe we don't auto-focus search?
                // But the user might want to add more. Let's keep behavior consistent or check.
                setTimeout(() => {
